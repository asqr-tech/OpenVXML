...
public class MetaDataItemConfiguration implements IConfiguration, InteractionsConstants
{
	...
	/**
	 * Sets the value of the target variable as the result of an
	 * expression.
	 * 
	 * @param expression        The expression to evaluate.
	 * @param scriptingLanguage The scripting language the expression is
	 *                          in.
	 */
	 ...
	 * 
	 * @param variableName The name of the variable to set as the target
	 *                     variable.
	 */
	...
	public void load(Element configurationElement)
	{
		...
		if (typeString.equals("")) //$NON-NLS-1$
			type = TYPE_STATIC;
		else if ("static".equalsIgnoreCase(typeString)) //$NON-NLS-1$
			type = TYPE_STATIC;
		else if ("expression".equalsIgnoreCase(typeString)) //$NON-NLS-1$
			type = TYPE_EXPRESSION;
		else if ("variable".equalsIgnoreCase(typeString)) //$NON-NLS-1$
			type = TYPE_VARIABLE;
	...
	public void save(Element configurationElement)
	{
		...
		if (scriptingLanguage != null) configurationElement.setAttribute(NAME_SCRIPTING_LANGUGAGE, scriptingLanguage);
	}
}
