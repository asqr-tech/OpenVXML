...
public class DatabaseRegistry implements IDatabaseRegistry
{
	 ...
	 * 
	 * @see
	 * org.eclipse.vtp.framework.databases.IDatabaseRegistry#getDatabase(
	 * java.lang.String)
	 */
		...
		AbstractDatabase(IDataTypeRegistry types, DatabaseConfiguration configuration)
		{
			...
			for (int i = 0; i < tableConfigs.length; ++i)
			{
				...
				for (int j = 0; j < columnConfigs.length; ++j)
				{
					...
					if (type != null) table.put(columnConfigs[j].getName(), types.getDataType(type));
				}
	...
	private final class JdbcDatabase extends AbstractDatabase
	{
		...
		public Connection getConnection() throws SQLException
		{
			...
			if (username != null) info.setProperty("user", username); //$NON-NLS-1$
			if (password != null) info.setProperty("password", password); //$NON-NLS-1$
	...
	private final class JndiDatabase extends AbstractDatabase
	{
		...
		public Connection getConnection() throws SQLException
		{
			...
			if (username != null && password != null) return dataSource.getConnection(username, password);
