...
public class DatabaseQueryAction implements IAction
{
	 ...
	 */
	public DatabaseQueryAction(IActionContext context, IDataTypeRegistry types, IVariableRegistry variables, IDatabaseRegistry databases,
		DatabaseQueryConfiguration configuration)
	{
		...
		if (configuration.isResultArray())
			this.resultLength = configuration.getResultLimit() <= 0 ? Integer.MAX_VALUE : configuration.getResultLimit();
		else
			this.resultLength = 0;
		...
		for (int i = 0; i < columnNames.length; ++i)
			columns.put(columnNames[i], database.getColumnType(table, columnNames[i]));
	...
	public IActionResult execute()
	{
		...
		{
			if (result == null) result = context.createResult("error.database.connection", e); //$NON-NLS-1$
		}
		if (result == null) context.createResult("error.database.connection"); //$NON-NLS-1$
		return result;
	}
	...
	private PreparedStatement buildStatement(Connection connection) throws SQLException
	{
		...
		if (buffer.length() == length) buffer.append(' ').append('*');
		...
		if (!criteria.isEmpty())
		{
			...
			if (buffer.length() == length) buffer.setLength(length - 7);
		}
		// buffer.append(';');
		Dictionary props = new Hashtable();
		...
		context.report(IReporter.SEVERITY_INFO, "Executing SQL statement: " + (criteriaIsSecured ? "**Secured**" : buffer.toString()), props);
	...
	private IActionResult processResults(ResultSet resultSet) throws SQLException
	{
		...
		if (resultLength == 0)
		{
			if (variable == null || !resultType.equals(variable.getType()))
				variables.setVariable(resultName, variable = variables.createVariable(resultType));
		}
		else
		{
			if (variable == null || !arrayType.equals(variable.getType()))
				variables.setVariable(resultName, array = (IArrayObject) variables.createVariable(arrayType));
			else
		...
		{
			resultCount++;
			if (variable == null) variable = variables.createVariable(resultType);
			for (Mapping mapping : mappings)
	...
	private final class Criteria
	{
		...
		{
			if (comparison == null || value == null) return;
			if (and) sql.append(" and "); //$NON-NLS-1$
		...
		{
			if (comparison == null || value == null) return index;
			if (value instanceof IBooleanObject)
				statement.setBoolean(index, ((IBooleanObject) value).getValue().booleanValue());
			else if (value instanceof IDateObject)
				statement.setTimestamp(index, new Timestamp(((IDateObject) value).getValue().getTime().getTime()));
			else if (value instanceof IDecimalObject)
	...
	private final class Mapping
	{
		...
		void setValue(IDataObject object, ResultSet resultSet) throws SQLException
		{
			...
			if (target.getType().isComplexType()) target = target.getField(name);
			Object data = value;
			if (type != null) data = resultSet.getObject(value);
			if (target instanceof IBooleanObject)
				((IBooleanObject) target).setValue(data);
			else if (target instanceof IDateObject)
			{
				if (data instanceof java.sql.Date)
					((IDateObject) target).setValue(new Date(((java.sql.Date) data).getTime()));
			...
			else if (target instanceof IStringObject) ((IStringObject) target).setValue(data);
		}
	}
}
