...
public class ResourceGroup implements IResourceManager, ExternalServerManagerListener {
		...
		if (indexURL != null) {
			try {
				BufferedReader br = new BufferedReader(
						new InputStreamReader(indexURL.openConnection().getInputStream()));
						...
						if (locations.size() > 0) {
							...
							for (ExternalServer server : locations) {
								...
								try {
									...
									BufferedReader br = new BufferedReader(
											new InputStreamReader(new ByteArrayInputStream(responseBody.getBytes())));
									String line = br.readLine();
									while (line != null) {
										if (logging == ExternalServerManager.Logging.ALWAYS)
	...
	public String[] listResources(String fullDirectoryPath) {
		...
		for (Enumeration<String> e = bundle.getEntryPaths(path + fullDirectoryPath); e != null && e.hasMoreElements();)
