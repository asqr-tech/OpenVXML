...
public class Blueprint {
	...
	private Map indexConfigurations(ConfigurationDescriptor[] configurationDescriptors) {
		Map configurationDescriptorIndex = new HashMap(configurationDescriptors.length);
		for (int i = 0; i < configurationDescriptors.length; ++i) {
			...
			LinkedList list = (LinkedList) configurationDescriptorIndex.get(xmlIdentifier);
			if (list == null)
				configurationDescriptorIndex.put(xmlIdentifier, list = new LinkedList());
	 ...
	 * @return A Map of service lists indexed by identifiers.
	 */
	private Map buildExecutables(IProcessDefinition definition, ActionDescriptor[] actionDescriptors,
