...
public class VoicePlatform extends AbstractPlatform implements VXMLConstants
{
	...
	public Locale getCurrentLocale()
	{
		if (mediaProvider == null)
		{
			...
			ILanguageSelection languageSelection = (ILanguageSelection) context.lookup(ILanguageSelection.class.getName());
			String languageID = languageSelection.getSelectedLanguage();
			IBrandSelection brandSelection = (IBrandSelection) context.lookup(IBrandSelection.class.getName());
			IBrand brand = brandSelection.getSelectedBrand();
			IMediaProviderRegistry mediaProviderRegistry = (IMediaProviderRegistry) context.lookup(IMediaProviderRegistry.class.getName());
	...
	public AudioOutput generateAudioChain(ILinkFactory links, String path)
	{
		...
		for (int i = 0; i < servers.size(); i++)
		{
			...
			if (serverPrefix != null)
			{
				...
				if (!serverPrefix.endsWith("/") && !path.startsWith("/")) fullPath = serverPrefix + "/" + path;
				if (i == 0)
				{
	 ...
	 * @return        A new VXML document that contains the supplied
	 *                dialog.
	 */
	 ...
	 * AbstractPlatform# renderOutputMessage(
	...
	protected IDocument renderOutputMessage(ILinkFactory links, OutputMessageCommand outputMessageCommand)
	{
		...
		if (bargeIn != null) form.setProperty(NAME_BARGEIN, bargeIn);
		...
		for (int i = 0; i < parameterNames.length; ++i)
			nextLink.setParameters(parameterNames[i], outputMessageCommand.getParameterValues(parameterNames[i]));
		...
		for (int i = 0; i < parameterNames.length; ++i)
			hangupLink.setParameters(parameterNames[i], outputMessageCommand.getParameterValues(parameterNames[i]));
	 ...
	 * 
	 * @see org.eclipse.vtp.framework.interactions.core.support.
	 * AbstractPlatform# renderInitialDocument(
	 * org.eclipse.vtp.framework.interactions.core.platforms.ILinkFactory,
	 * org.eclipse.vtp.framework.interactions.core.commands.
	 * InitialCommand)
	 */
	 ...
	 * 
	 * @see org.eclipse.vtp.framework.interactions.core.support.
	 * AbstractPlatform# renderInputRequest(
	 * org.eclipse.vtp.framework.interactions.core.platforms.ILinkFactory,
	 * org.eclipse.vtp.framework.interactions.core.commands.
	 * InputRequestCommand)
	 */
	protected IDocument renderInputRequest(ILinkFactory links, InputRequestCommand inputRequestCommand)
	{
		...
		TimeValue maxSpeechLength = resolveTimeValue("max-speech-timeout", inputRequestCommand.getPropertyValue("max-speech-timeout")); //$NON-NLS-2$
		String maxNBest = inputRequestCommand.getPropertyValue("max-n-best"); //$NON-NLS-1$
		TimeValue interDigitTimeout = resolveTimeValue("interdigit-timeout", inputRequestCommand.getPropertyValue("interdigit-timeout")); //$NON-NLS-2$
		TimeValue terminationTimeout = resolveTimeValue("termination-timeout", inputRequestCommand.getPropertyValue("termination-timeout")); //$NON-NLS-2$
		...
		else
		{
			try
			{
				if (confidenceLevel != null && confidenceLevel.length() > 0)
					field.setProperty(NAME_CONFIDENCELEVEL, new BigDecimal(confidenceLevel).divide(ONE_HUNDRED).toString());
			}
			...
			try
			{
				if (speedVsAccuracy != null && speedVsAccuracy.length() > 0)
					field.setProperty(NAME_SPEEDVSACCURACY, new BigDecimal(speedVsAccuracy).divide(ONE_HUNDRED).toString());
			}
			catch (NumberFormatException e)
			{
				e.printStackTrace();
	 ...
	 * 
	 * @see org.eclipse.vtp.framework.interactions.core.support.
	 * AbstractPlatform# renderSelectionRequest(
	 * org.eclipse.vtp.framework.interactions.core.platforms.ILinkFactory,
	 * org.eclipse.vtp.framework.interactions.core.commands.
	...
	protected IDocument renderSelectionRequest(ILinkFactory links, SelectionRequestCommand selectionRequestCommand)
	{
		...
		TimeValue timeout = resolveTimeValue("initial-timeout", selectionRequestCommand.getPropertyValue("initial-timeout")); //$NON-NLS-2$
		...
		TimeValue maxSpeechLength = resolveTimeValue("max-speech-timeout", selectionRequestCommand.getPropertyValue("max-speech-timeout")); //$NON-NLS-2$
		String maxNBest = selectionRequestCommand.getPropertyValue("max-n-best"); //$NON-NLS-1$
		TimeValue interDigitTimeout = resolveTimeValue("interdigit-timeout", selectionRequestCommand.getPropertyValue("interdigit-timeout")); //$NON-NLS-2$
		TimeValue terminationTimeout = resolveTimeValue("termination-timeout", selectionRequestCommand.getPropertyValue("termination-timeout")); //$NON-NLS-2$
		...
		if (bargeIn != null) menu.setProperty(NAME_BARGEIN, bargeIn);
		if (timeout != null) menu.setProperty(NAME_TIMEOUT, timeout.toTimeString(TimeValue.SECONDS)); //$NON-NLS-1$
		if ("hybrid".equalsIgnoreCase(inputMode))
		{
		...
		else
		{
			...
			{
				if (confidenceLevel != null && confidenceLevel.length() > 0)
					menu.setProperty(NAME_CONFIDENCELEVEL, new BigDecimal(confidenceLevel).divide(ONE_HUNDRED).toString());
			}
			catch (NumberFormatException e)
			{
				e.printStackTrace();
			...
			{
				if (sensitivity != null && sensitivity.length() > 0)
					menu.setProperty(NAME_SENSITIVITY, new BigDecimal(sensitivity).divide(ONE_HUNDRED).toString());
			}
			catch (NumberFormatException e)
			{
				e.printStackTrace();
			...
			{
				if (speedVsAccuracy != null && speedVsAccuracy.length() > 0)
					menu.setProperty(NAME_SPEEDVSACCURACY, new BigDecimal(speedVsAccuracy).divide(ONE_HUNDRED).toString());
			}
			catch (NumberFormatException e)
			{
				e.printStackTrace();
			}
			if (speechCompletionTimeout != null) menu.setProperty(NAME_COMPLETETIMEOUT, speechCompletionTimeout.toTimeString(TimeValue.SECONDS)); //$NON-NLS-1$
			if (speechIncompleteTimeout != null) menu.setProperty(NAME_INCOMPLETETIMEOUT, speechIncompleteTimeout.toTimeString(TimeValue.SECONDS)); //$NON-NLS-1$
			if (maxSpeechLength != null)
			...
			if (maxNBest != null && maxNBest.length() > 0) menu.setProperty(NAME_MAXNBEST, maxNBest);
		}
		if ("voice only".equalsIgnoreCase(inputMode)) //$NON-NLS-1$
		{
		...
		{
			if (interDigitTimeout != null) menu.setProperty(NAME_INTERDIGITTIMEOUT, interDigitTimeout.toTimeString(TimeValue.SECONDS)); //$NON-NLS-1$
			if (terminationTimeout != null) menu.setProperty(NAME_TERMTIMEOUT, terminationTimeout.toTimeString(TimeValue.SECONDS)); //$NON-NLS-1$
			if (terminationCharacter != null && terminationCharacter.length() > 0 && !"none".equalsIgnoreCase(terminationCharacter)) //$NON-NLS-1$
		...
		for (int i = 0; i < selectionRequestCommand.getOptionCount(); ++i)
		{
				...
				switch (selectionRequestCommand.getOptionInputType(i))
				{
					case SelectionRequestCommand.INPUT_TYPE_FILE:
						choice.setGrammar(new ExternalGrammar(GRAMMAR_MODE_VOICE,
							links.createResourceLink(selectionRequestCommand.getOptionInputValue(i)).toString()));
						break;
					case InputRequestCommand.INPUT_TYPE_INLINE:
			...
			if (!GRAMMAR_MODE_VOICE.equals(grammarMode)) choice.setDTMF(dtmf == null ? "0" : dtmf);
	 ...
	 * 
	 * @see org.eclipse.vtp.framework.interactions.core.support.
	 * AbstractPlatform# renderDataRequest(
	 * org.eclipse.vtp.framework.interactions.core.platforms.ILinkFactory,
	 * org.eclipse.vtp.framework.interactions.core.commands.
	 * DataRequestCommand)
	 */
	protected IDocument renderDataRequest(ILinkFactory links, DataRequestCommand dataRequestCommand)
	{
		...
		String dtmfTerm = getNormalizedBoolean(dataRequestCommand.getPropertyValue("dtmf-termination")); //$NON-NLS-1$
		...
		if (playBeep != null) recording.setBeepEnabled(Boolean.valueOf(playBeep).booleanValue());
		if (dtmfTerm != null) recording.setDtmfTermEnabled(Boolean.valueOf(dtmfTerm).booleanValue());
		if (initialTimeout != null && initialTimeout.length() > 0) recording.setTimeout(initialTimeout + "s"); //$NON-NLS-1$
		if (finalSilence != null && finalSilence.length() > 0) recording.setFinalSilence(finalSilence + "s"); //$NON-NLS-1$
		...
		for (int i = 0; i < parameterNames.length; ++i)
		{
			...
			{
				buf.append(values[v]);
				if (v < values.length - 1) buf.append(',');
			}
		...
		for (int i = 0; i < parameterNames.length; ++i)
		{
			noInputLink.setParameters(parameterNames[i], dataRequestCommand.getParameterValues(parameterNames[i]));
		}
		...
		ILink hangupLink = links.createNextLink();
		for (int i = 0; i < parameterNames.length; ++i)
			hangupLink.setParameters(parameterNames[i], dataRequestCommand.getParameterValues(parameterNames[i]));
		Catch disconnectCatch = new Catch("connection.disconnect.hangup");
	...
	protected IDocument renderExternalReference(ILinkFactory links, ExternalReferenceCommand externalReferenceCommand)
	{
		...
		for (int i = 0; i < sourceParameters.length; i++)
		{
			...
			if (i != sourceParameters.length - 1) nameListBuffer.append(' ');
		}
		...
		for (int i = 0; i < outputArgNames.length; ++i)
		{
			...
			filled.addVariable(new Variable(outputArgNames[i], externalReferenceCommand.getReferenceName() + "." + externalReferenceCommand //$NON-NLS-1$
		...
		ILink hangupLink = links.createNextLink();
		for (int i = 0; i < parameterNames.length; ++i)
			hangupLink.setParameters(parameterNames[i], externalReferenceCommand.getParameterValues(parameterNames[i]));
		...
		for (int i = 0; i < parameterNames.length; ++i)
			fetchLink.setParameters(parameterNames[i], externalReferenceCommand.getParameterValues(parameterNames[i]));
		fetchLink.setParameter(externalReferenceCommand.getResultName(), externalReferenceCommand.getBadFetchResultValue());
	 ...
	 * 
	 * @see org.eclipse.vtp.framework.interactions.core.support.
	 * AbstractPlatform# renderTransferMessage(
	 * org.eclipse.vtp.framework.interactions.core.platforms.ILinkFactory,
	 * org.eclipse.vtp.framework.interactions.core.commands.
	 ...
	 * 
	 * @see org.eclipse.vtp.framework.interactions.core.support.
	 * AbstractPlatform# renderBridgeMessage(
	 * org.eclipse.vtp.framework.interactions.core.platforms.ILinkFactory,
	 * org.eclipse.vtp.framework.interactions.core.commands.
	 ...
	 * 
	 * @see org.eclipse.vtp.framework.interactions.core.support.
	 * AbstractPlatform# renderEndMessage(
	 * org.eclipse.vtp.framework.interactions.core.platforms.ILinkFactory,
	 * org.eclipse.vtp.framework.interactions.core.commands.
	 * EndMessageCommand)
	 */
	 ...
	 * 
	 * @see org.eclipse.vtp.framework.interactions.core.support.
	 * AbstractPlatform# renderFinalDocument(
	 * org.eclipse.vtp.framework.interactions.core.platforms.ILinkFactory,
	...
	private Dialog addExtendedEvents(ILinkFactory links, String resultName, Map<String, String[]> parameterMap, Dialog form)
	{
		...
		if (false)
		{
			...
			for (String cpaEvent : cpaEvents)
			{
				...
				{
					ILink eventLink = links.createNextLink();
					if (null != parameterMap) for (Entry<String, String[]> entry : parameterMap.entrySet())
