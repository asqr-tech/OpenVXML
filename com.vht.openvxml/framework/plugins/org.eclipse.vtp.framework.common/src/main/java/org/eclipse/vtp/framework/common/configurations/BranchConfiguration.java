...
public class BranchConfiguration implements IConfiguration, CommonConstants
{
	 ...
	 * operation.
	 */
	private String leftScriptingLanguage = null; //$NON-NLS-1$
	 ...
	 * operation.
	 */
	private String rightScriptingLanguage = null; //$NON-NLS-1$
	 ...
	 * 
	 * @param leftValue The name of the variable to use for the left-hand
	 ...
	 * Sets the value to use for the left-hand side of the operation.
	 * 
	 * @param leftValue             The expression to use for the
	 *                              left-hand side of the operation.
	 * @param leftScriptingLanguage The scripting language to use for the
	 *                              left-hand side of the operation.
	 */
	 ...
	 * 
	 * @param rightValue The name of the variable to use for the
	 ...
	 * Sets the value to use for the right-hand side of the operation.
	 * 
	 * @param rightValue             The expression to use for the
	 *                               right-hand side of the operation.
	 * @param rightScriptingLanguage The scripting language to use for the
	 *                               right-hand side of the operation.
	 */
	...
	public void load(Element configurationElement)
	{
		...
		NodeList leftList = configurationElement.getElementsByTagNameNS(NAMESPACE_URI, NAME_LEFT_OPERAND);
		if (leftList.getLength() > 0)
		{
			Element left = (Element) leftList.item(0);
			if ("expression".equalsIgnoreCase(left.getAttribute(NAME_TYPE))) //$NON-NLS-1$
			...
			if (left.hasAttribute(NAME_SCRIPTING_LANGUGAGE)) leftScriptingLanguage = left.getAttribute(NAME_SCRIPTING_LANGUGAGE);
		}
		rightType = OPERAND_TYPE_VARIABLE;
		rightValue = ""; //$NON-NLS-1$
		rightScriptingLanguage = null;
		NodeList rightList = configurationElement.getElementsByTagNameNS(NAMESPACE_URI, NAME_RIGHT_OPERAND);
		if (rightList.getLength() > 0)
		{
			Element right = (Element) rightList.item(0);
			if ("expression".equalsIgnoreCase(right.getAttribute(NAME_TYPE))) //$NON-NLS-1$
			...
			if (right.hasAttribute(NAME_SCRIPTING_LANGUGAGE)) rightScriptingLanguage = right.getAttribute(NAME_SCRIPTING_LANGUGAGE);
		}
	}
	...
	public void save(Element configurationElement)
	{
		...
		Element left = configurationElement.getOwnerDocument().createElementNS(NAMESPACE_URI, leftName);
		...
		if (leftScriptingLanguage != null) left.setAttribute(NAME_SCRIPTING_LANGUGAGE, leftScriptingLanguage);
		configurationElement.appendChild(left);
		Element right = configurationElement.getOwnerDocument().createElementNS(NAMESPACE_URI, rightName);
		...
		if (rightScriptingLanguage != null) right.setAttribute(NAME_SCRIPTING_LANGUGAGE, rightScriptingLanguage);
