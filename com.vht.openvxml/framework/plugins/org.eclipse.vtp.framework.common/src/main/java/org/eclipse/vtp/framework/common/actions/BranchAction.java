...
public class BranchAction implements IAction
{
	 ...
	 */
	public BranchAction(IActionContext context, IVariableRegistry variableRegistry, IScriptingService scriptingService,
	...
	public IActionResult execute()
	{
		try
		{
			if (context.isInfoEnabled()) context.info("Performing comparison(s)");
			for (int i = 0; i < configurations.length; ++i)
			{
				Object left = evaluate(configurations[i].getLeftType(), configurations[i].getLeftValue(),
					configurations[i].getLeftScriptingLanguage());
				Object right = evaluate(configurations[i].getRightType(), configurations[i].getRightValue(),
					configurations[i].getRightScriptingLanguage());
				boolean result = compare(left, right, configurations[i]);
				if (context.isReportingEnabled())
				{
					Dictionary<String, Object> props = new Hashtable<String, Object>();
					props.put("event", "branch");
					props.put("branch.left", isSecured(left, configurations[i].isLeftSecured()) ? "**Secured**" : String.valueOf(left));
					props.put("branch.operator", COMPARISONS[configurations[i].getType()]);
					props.put("branch.right", isSecured(right, configurations[i].isRightSecured()) ? "**Secured**" : String.valueOf(right));
					props.put("branch.result", String.valueOf(result));
					context.report(IReporter.SEVERITY_INFO, "Comparison \"" + configurations[i].getLeftValue() + "\" "
						+ COMPARISONS[configurations[i].getType()] + " \"" + configurations[i].getRightValue() + "\" evaluated to " + result, props);
				}
				if (result) return context.createResult(configurations[i].getPath());
			}
	 ...
	 * @param  scriptingLanguage The scripting language to use if the
	...
	private Object evaluate(int operandType, String operandValue, String scriptingLanguage)
	{
		if (operandType == BranchConfiguration.OPERAND_TYPE_VARIABLE)
		{
			...
			{
				List<ILastResultData> results = lastResult.getResults();
	...
	private boolean compare(Object left, Object right, BranchConfiguration configuration)
	{
		...
		if (context.isInfoEnabled()) context.info("Left object: " + (left == null ? "null" : left.getClass().getName()) + " ("
			+ (isSecured(left, configuration.isLeftSecured()) ? "**Secured**" : String.valueOf(left)) + ") Right object: "
			+ (right == null ? "null" : right.getClass().getName()) + " ("
			+ (isSecured(right, configuration.isRightSecured()) ? "**Secured**" : String.valueOf(right)) + ")");
		...
		else if (right instanceof IDataObject)
		{
			...
			return comparison == BranchConfiguration.COMPARISON_TYPE_EQUAL || comparison == BranchConfiguration.COMPARISON_TYPE_LESS_THAN_OR_EQUAL
	...
	/**
	 * Suppress warnings because we know both objects are the same
	 * concrete type.
	 * 
