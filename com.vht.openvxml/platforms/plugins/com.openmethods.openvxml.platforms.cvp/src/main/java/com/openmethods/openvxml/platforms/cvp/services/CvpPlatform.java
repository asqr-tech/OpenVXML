...
public class CvpPlatform extends VoicePlatform
{
	...
	protected IDocument renderInputRequest(ILinkFactory links, InputRequestCommand inputRequestCommand)
	{
					...
					if ("dtmf only".equalsIgnoreCase(inputMode))
					{
						...
						else
						{
							...
							if (lastExtensionIndex != -1)
							{
								...
								URL url = manager
									.getResource((input.getProperty("media-library") == null ? "" : input.getProperty("media-library") + "/") + path
										+ lastPart + query);
								if (url == null) url = manager.getResource("Default/" + path + lastPart + query);
								System.out.println("resource URL: " + url);
								if (url == null)
								{
									field.addGrammar(
										new ExternalGrammar("dtmf", links.createResourceLink(inputRequestCommand.getInputValue()).toString()));
								}
								else
								{
									try
		...
		noInput
			.addAction(new Submit(noInputLink.toString(), new String[] { inputRequestCommand.getResultName(), inputRequestCommand.getDataName() }));
