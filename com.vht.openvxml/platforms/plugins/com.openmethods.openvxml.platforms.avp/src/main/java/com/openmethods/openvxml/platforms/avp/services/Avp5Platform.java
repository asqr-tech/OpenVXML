...
public class Avp5Platform extends VoicePlatform
{
	 ...
	 * AbstractPlatform# createDocument(
	...
	protected IDocument renderInputRequest(ILinkFactory links, InputRequestCommand inputRequestCommand)
	{
		...
		String speechCompletionTimeout = inputRequestCommand.getPropertyValue("speech-complete-timeout"); //$NON-NLS-1$
		String speechIncompleteTimeout = inputRequestCommand.getPropertyValue("speech-incomplete-timeout"); //$NON-NLS-1$
		...
		if (timeout != null && timeout.length() > 0) field.setProperty(NAME_TIMEOUT, timeout + "s"); //$NON-NLS-1$
		...
		{
			if (interDigitTimeout != null && interDigitTimeout.length() > 0) field.setProperty(NAME_INTERDIGITTIMEOUT, interDigitTimeout + "s"); //$NON-NLS-1$
			if (terminationTimeout != null && terminationTimeout.length() > 0) field.setProperty(NAME_TERMTIMEOUT, terminationTimeout + "s"); //$NON-NLS-1$
		...
		filled.addAction(new Submit(filledLink.toString(), new String[] { inputRequestCommand.getDataName(), "lastresult" }));
		...
		//				"		}\r\n" +
		//				"		lastresult = lastresult + '</lastresult>';\r\n");
		noMatch.addAction(new Submit(noMatchLink.toString(), new String[] { inputRequestCommand.getDataName(), "lastresult" }));
	...
	protected IDocument renderSelectionRequest(ILinkFactory links, SelectionRequestCommand selectionRequestCommand)
	{
		...
		String speechCompletionTimeout = selectionRequestCommand.getPropertyValue("speech-complete-timeout"); //$NON-NLS-1$
		String speechIncompleteTimeout = selectionRequestCommand.getPropertyValue("speech-incomplete-timeout"); //$NON-NLS-1$
