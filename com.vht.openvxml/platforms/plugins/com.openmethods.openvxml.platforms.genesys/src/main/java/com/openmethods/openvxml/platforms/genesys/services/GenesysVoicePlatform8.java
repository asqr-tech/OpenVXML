...
public class GenesysVoicePlatform8 extends VoicePlatform {
	...
	protected IDocument renderInitialDocument(ILinkFactory links, InitialCommand initialCommand) {
		...
		jsonInclude.setSrc(
				links.createIncludeLink(Activator.getDefault().getBundle().getSymbolicName() + "/includes/json.js")
						.toString());
...
//					If eventIf = new If("_event=='" + cpaEvent + "'");
			...
			for (int i = 0; i < parameterNames.length; ++i)
				cpaLink.setParameters(parameterNames[i], initialCommand.getParameterValues(parameterNames[i]));
	...
	protected IDocument renderMetaDataRequest(ILinkFactory links, MetaDataRequestCommand metaDataMessageRequest) {
		...
		for (int i = 0; i < names.length; i++) {
			...
			String encodedValue;
			try {
				encodedValue = URLEncoder.encode(names[i], StandardCharsets.UTF_8.toString());
...
//		block.addAction(new Assignment("GetDataMessage", "application.lastmessage$.content"));
		 ...
		 * metaDataMessageRequest.getMetaDataNames(); for(int i = 0; i < names.length;
		 * i++) { userData.addParameter(new Parameter(names[i], "''")); } String[]
		 * parameterNames = metaDataMessageRequest.getParameterNames(); String[]
		 * submitVars = new String[parameterNames.length + 2]; submitVars[0] =
		 ...
		 * parameterNames.length; ++i) { submitVars[i + 2] = parameterNames[i]; String[]
		 * values = metaDataMessageRequest.getParameterValues(parameterNames[i]);
		 * StringBuffer buf = new StringBuffer(); for(int v = 0; v < values.length; v++)
		 * { buf.append(values[v]); if(v < values.length - 1) buf.append(','); }
		 * Variable paramVar = new Variable(parameterNames[i], "'" + buf.toString() +
		 * "'"); filled.addVariable(paramVar); } ILink filledLink =
		 * links.createNextLink(); Submit submit = new Submit(filledLink.toString(),
		 * submitVars); submit.setMethod(VXMLConstants.METHOD_POST);
		 * submit.setEncodingType("multipart/form-data"); filled.addAction(submit);
		 * userData.addFilledHandler(filled); form.addFormElement(userData); ILink
		 * hangupLink = links.createNextLink(); for (int i = 0; i <
		 * parameterNames.length; ++i) hangupLink.setParameters(parameterNames[i],
	...
	protected IDocument renderMetaDataMessage(ILinkFactory links, MetaDataMessageCommand metaDataMessageCommand) {
		...
		for (int i = 0; i < names.length; i++) {
			String encodedName;
			try {
				encodedName = URLEncoder.encode(names[i], StandardCharsets.UTF_8.toString());
...
//		send.setBody(nameList.toString() + (isCtiC ? "&Action=AttachData&sub_action=Add": ""));
	...
	public Map processMetaDataResponse(MetaDataRequestConfiguration configuration, IActionContext context) {
		...
		try {
			...
			JsonParser jp = jsonFactory.createParser(attachedDataContent);
			...
			if (userData.containsKey("content")) {
				...
				for (String kvpString : contentArray) {
					...
					dataMap.put(kvpArray[0],
							URLDecoder.decode((String) kvpArray[1], StandardCharsets.UTF_8.toString()));
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
		 ...
		 * context.debug("AttachedDataDocument: " + attachedDataDocument); NodeList
		 * dataList =
		 * attachedDataDocument.getDocumentElement().getElementsByTagName("key");
		 * for(int i = 0; i < dataList.getLength(); i++) { Element dataElement =
		 * (Element)dataList.item(i); context.debug("KVP received - key: " +
