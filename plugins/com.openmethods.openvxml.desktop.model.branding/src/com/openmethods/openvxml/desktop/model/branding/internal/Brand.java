...
public class Brand implements IBrand {
	...
	/**
	 * Sets the name of this brand to the given value. If the new name is equal to
	 * the old name, no action is taken. Otherwise, the new name is made the current
	 * name and a property change event is propagated to this brand's manager.
	 * 
	...
	/**
	 * Sets the given brand as the parent of this brand. If the current parent is
	 * the same as the new parent, no action is taken. Prior to setting the new
	 * parent, the brand hierarchy is checked to ensure a cycle has not been
	 * introduced. If a cycle is detected, an illegal argument exception is thrown.
	 * This brand is added to the parent brand as a child. If this brand did not
	 * have a parent previously, a brand added event is dispatched to the brand
	 * manager. Otherwise, a brand parent changed event is delivered instead.
	 * 
	 * @param brand The new parent of this brand
	 * @throws IllegalArgumentException If the parent brand is null or applying the
	 *                                  new structure would result in a cycle in the
	 *                                  hierarchy.
	 */
	 ...
	 * list of child brands. The manager which contains this brand is set into the
	 * child brand.
	 * 
	...
	/**
	 * Removes the given brand from the list of this brand's child brands. If the
	 * given brand was not this brand's child no action is taken.
	 * 
	...
	/**
	 * Ensures the given brand is eligible to be this brand's parent. An exception
	 * is raised if a circular structure would be created by adding this brand to
	 * the given one.
	 * 
	...
	/**
	 * Removes this brand from the current brand structure. This will remove this
	 * brand from its parent and also delete all of this brand's child brands.
	 */
	public void delete() {
		for (int i = 0; i < childBrands.size(); i++) {
